...
class GameRepositoryYongHan
{
    ...
    {
        return $this->getPdo()->prepare('SELECT * FROM games')
    ...
    {
        if (empty($user) || empty($game_name) || empty($game_desc) || empty($game_publisher)
            || empty($game_datetime) || empty($game_year) || empty($game_cover)) {
            return false;
        } else {
            $this->getPdo()->prepare('INSERT INTO games (user_id, game_name, game_desc, 
    ...
    {
        return array('user_id' => 1, 'game_id' => 1, 'game_name' => 'Pokemon Sword',
            'game_description' => 'Explore a vast world and catch creatures!', 'game_publisher' => 'GameFreak',
            'game_datetime' => '2021-10-15 22:00:00', 'game_year' => '2020', 'game_cover' => 'pokemon_cover.jpg');
    }
